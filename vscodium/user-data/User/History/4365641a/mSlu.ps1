...
$vhdxPath ="C:\MyVM\MyVM.vhdx"

if (Test-Path -Path $vhdxPath) {
    # Optimize virtual machine disk size
    Convert-VHD -Path $vhdxPath -DestinationPath $vhdxPath -Mode Dynamic
    # Utilize thin provisioning
    New-VHD -Dynamic -Path $vhdxPath -SizeBytes 32GB
    # Optimize virtual machine disk size
    $vmName = "<Your VM Name>"
    $vm = Get-VM -Name $vmName
    $vmDiskPath = ($vm | Select-Object -ExpandProperty VirtualDisks)[0].Path
