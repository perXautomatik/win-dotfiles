...
foreach ($SubFolder1 in $FileCounts.Keys) {
    ...
    foreach ($SubFolder2 in $FileCounts.Keys) {
        ...
        # Skip if the subfolder1 has is a possible child of subfolder2
...
$SortedMatches = $FileMatches.GetEnumerator() | ? { $_.Value -gt 1 } | Sort-Object -Property Value -Descending
