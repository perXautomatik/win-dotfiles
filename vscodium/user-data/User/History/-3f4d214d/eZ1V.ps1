...
foreach ($Group in $Groups) { 
    ...
    $FileHashes[$SubFolder] = Get-FileHash -Path $Group.Group.FullName -Algorithm SHA256 
}
<# 
...
$SortedHashMatches = $FileHashMatches.GetEnumerator() | Where-Object { $.Value.Count -GT 0 } | Sort-Object -Property @{Expression = { $.Value.Count }; Descending = $true }
