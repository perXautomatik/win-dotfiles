...
foreach ($Group in $Groups) { # Get the subfolder name $SubFolder = $Group.Name # Get the file hash objects for the subfolder $FileHashes[$SubFolder] = Get-FileHash -Path $Group.Group.FullName -Algorithm SHA256 }
...
foreach ($SubFolder2 in $FileHashes.Keys) {
    ...
    $FileHashMatches[$Pair] = Compare-Object -ReferenceObject $FileHashes[$SubFolder1] -DifferenceObject $FileHashes[$SubFolder2] -Property Hash -PassThru -IncludeEqual | Where-Object { $_.SideIndicator -eq "==" }
}

}

Sort the file hash matches by count in descending order
$SortedHashMatches = $FileHashMatches.GetEnumerator() | ? { $.Value.Count -gt 0 } | Sort-Object -Property @{Expression={$.Value.Count}; Descending=$true}
