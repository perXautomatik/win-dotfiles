...
foreach ($commit in $commits) {
    $diff = git show $commit -p -- :CollectionHolder.cs
    if ($diff | Select-String -Pattern "^\+.*CheckGlobalStock" -SimpleMatch -CaseSensitive) {
        $hashMap[$commit] = $diff
    }
}

$outputs = $hashMap.GetEnumerator() | Out-GridView -Title "Select an item" -OutputMode Single
